...
def gui_sendstats(board_obj, piece_pos):
#    newdict = {}
#    piece_obj = board_obj.squares[piece_pos]
#    newdict['']
#
...
class Board(object):
  ...
  +-------------------------+\n""" % files
    ...
    def advance_turn(self):  # this is the refresh/update function
        ...
        for piece, vector in self.pinned_pieces.iteritems():  # prune moves for pinned pieces
                ...
                try:
                    ...
                    print "PROBLEM finding available moves of a piece"
                    print "piece: "
                    print piece
                    print "position:"
    ...
    def play(self, instring):  # this is a really ugly function but it's just for the user
        # takes algebraic chess notation as input, makes those moves from player side
                ...
                if new_piece_class is not None:
                    ...
                    if newsquare[1] == 8 or newsquare[1] == 1:
                        ...
                        try:
                            ...
                            self.playermessage("That doesn't seem to be a valid move")
                done = True
        elif len(instring) == 3:
    ...
    def rewind(self):  # opposite of advance turn
        ...
        if 'capture' in movetype:
            ...
            if self.whose_turn == 'white':
                ...
                rook_new_pos = (6,1)
                rookpiece = self.squares[rook_new_pos]
            else:
                rook_original_pos = (8, 8)
                rook_new_pos = (6,8)
                rookpiece = self.squares[rook_new_pos]
            self.squares[rook_new_pos] = None
            rookpiece.pos = rook_original_pos
            self.squares[rook_original_pos] = rookpiece
            rookpiece.has_moved = False
            piece.has_moved = False
        elif movetype == 'qside':
            if self.whose_turn == 'white':
                rook_original_pos = (1, 1)
                rook_new_pos = (4,1)
                rookpiece = self.squares[rook_new_pos]
            else:
                rook_original_pos = (1, 8)
                rook_new_pos = (4,8)
                rookpiece = self.squares[rook_new_pos]
            self.squares[rook_new_pos] = None
            rookpiece.pos = rook_original_pos

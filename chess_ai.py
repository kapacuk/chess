...
def lookahead(board, depth=2, max_depth=2): # simulates all possible moves for boards at an arbitrary depth
	...
	if depth > 0:
		for move in movelist:
			...
			if board.winner == None: # return the value of the best move up the tree
		...
		print (" "*depth) + "best move after %s at %s to %s is: %s at %s to %s" % (board.squares[board.movehistory[-1][0][1]], parse(board.movehistory[-1][0][0]), parse(board.movehistory[-1][0][1]), board.squares[bestmove[0]], parse(bestmove[0]), parse(bestmove[1]))
